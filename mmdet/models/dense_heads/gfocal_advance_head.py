...
class GFocalAdvanceHead(AnchorHead):
    ...
    def forward_single(self, x, scale):
        ...
        # bbox refinement 
        reg_refine = self.uncertainty_reg_branch(reg_feat, uncertainty.detach())
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, 
        ...
        return loss_cls, loss_bbox, loss_dfl, loss_bbox_offset, weight_targets.sum()

    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'bbox_offsets'))
    def loss(self,
             cls_scores,
             bbox_preds,
             bbox_offsets,   #####
        ...
        losses_cls, losses_bbox, losses_dfl, losses_bbox_offset,\
        ...
        return dict(
            loss_cls=losses_cls, loss_bbox=losses_bbox, loss_dfl=losses_dfl, loss_bbox_offset=losses_bbox_offset)
    ...
    def _get_bboxes_single(self,
        ...
        mlvl_bboxes = []
        mlvl_scores = []
        for cls_score, bbox_pred, bbox_offset, stride, anchors in zip( #####
                cls_scores, bbox_preds, bbox_offsets, self.anchor_generator.strides, #####
                mlvl_anchors):
