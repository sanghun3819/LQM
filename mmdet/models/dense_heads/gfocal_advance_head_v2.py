...
class GFocalAdvanceHeadv2(AnchorHead):
    ...
    def __init__(self,
        ...
        super(GFocalAdvanceHeadv2, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, cls_offset, bbox_offset, stride, anchors in zip( #####
            ...
            #########################################################################
            bbox_std = bboxes.detach().new_tensor([0.3, 0.3, 0.3, 0.3])
    ...
    def _get_target_single(self,
        ...
        bbox = bbox_preds[inside_flags, :]
        decode_inside_bbox_preds = distance2bbox(self.anchor_center(anchors), bbox, max_shape=img_shape) ### bbox --> (xmin, ymin, xmax, ymax)
        iou = pairwise_iou(Boxes(decode_inside_bbox_preds), Boxes(gt_bboxes))
        (max_iou, argmax_iou) = iou.max(dim=1)
        refine_pos = max_iou >= 0.6
        refine_neg = max_iou < 0.3
        refine_pos_inds = torch.nonzero(refine_pos, as_tuple=True)[0]
        refine_neg_inds = torch.nonzero(refine_neg, as_tuple=True)[0]

        bbox_refine_targets = torch.zeros_like(anchors)
        bbox_offset_targets = torch.zeros_like(anchors)
        bbox_offset_weights = torch.zeros_like(anchors)
        refine_labels = anchors.new_full((num_valid_anchors, ),
                                  self.num_classes,
                                  dtype=torch.long)
        refine_labels_weights = anchors.new_zeros(num_valid_anchors, dtype=torch.float)

        if len(refine_pos_inds) > 0:
            refine_bbox_targets = gt_bboxes[argmax_iou]
            bbox_refine_targets[refine_pos_inds] = refine_bbox_targets[refine_pos_inds]
            bbox_std = decode_inside_bbox_preds.new_tensor([0.3, 0.3, 0.3, 0.3])
            pre_boxes_wh = decode_inside_bbox_preds[refine_pos_inds, 2:4] - decode_inside_bbox_preds[refine_pos_inds, 0:2] ### [xmax - xmin, ymax - ymin] = [width, height]
            pre_boxes_wh = torch.cat([pre_boxes_wh, pre_boxes_wh], dim=1) ### [width, height, width, height]
            refine_bbox_offset_target = (refine_bbox_targets[refine_pos_inds] - decode_inside_bbox_preds[refine_pos_inds]) / (pre_boxes_wh * bbox_std)
            ...
            if gt_labels is None:
                ...
                gt_targets = gt_labels[argmax_iou]
                refine_labels[refine_pos_inds] = gt_targets[refine_pos_inds]
            if self.train_cfg.pos_weight <= 0:
                refine_labels_weights[refine_pos_inds] = 1.0
            else:
                refine_labels_weights[refine_pos_inds] = self.train_cfg.pos_weight
        
        else:
            # print('no assigned positive for refinement')
            pass

        if len(refine_neg_inds) > 0:
            refine_labels_weights[refine_neg_inds] = 1.0
        else:
            # print('no assigned negative for refinement')
            pass
        ###########################################################################
        # refine_label_weights = flat_anchors.new_zeros(flat_anchors.size(0), dtype=torch.float)
        # img_shape = img_meta['img_shape']
        # decoded_bbox_preds = distance2bbox(self.anchor_center(flat_anchors), bbox_preds, max_shape=img_shape)
        # iou = pairwise_iou(Boxes(decoded_bbox_preds), Boxes(gt_bboxes))
        # (max_iou, argmax_iou) = iou.max(dim=1)

        # # invalid = max_iou < 0.6
        # # refine_pos_inds = max_iou >= 0.6
        # valid = max_iou >= 0.6
        # invalid = max_iou < 0.6
        # valid_num = torch.nonzero(valid, as_tuple=True)[0].size(0)

        # gt_target = gt_bboxes[argmax_iou]
        # cls_refine_target = gt_labels[argmax_iou]
        # cls_refine_target[invalid] = self.num_classes
        # refine_label_weights[torch.nonzero(valid, as_tuple=True)[0]] = 1.0

        # bbox_std = decoded_bbox_preds.new_tensor([0.5 , 0.5, 0.5, 0.5])
        # pre_boxes_wh = decoded_bbox_preds[:, 2:4] - decoded_bbox_preds[:, 0:2]
        # pre_boxes_wh = torch.cat([pre_boxes_wh, pre_boxes_wh], dim=1)
        # bbox_offset_target = (gt_target - decoded_bbox_preds) / (pre_boxes_wh * bbox_std)
        ###########################################################################
        ...
        if unmap_outputs:
            ...
            label_weights= unmap(label_weights, num_total_anchors, inside_flags, fill=0)
            bbox_targets = unmap(bbox_targets, num_total_anchors, inside_flags, fill=0)
            bbox_weights = unmap(bbox_weights, num_total_anchors, inside_flags, fill=0)

            refine_labels         = unmap(refine_labels, num_total_anchors, inside_flags, fill=self.num_classes)
            refine_labels_weights = unmap(refine_labels_weights, num_total_anchors, inside_flags, fill=0)

            bbox_refine_targets   = unmap(bbox_refine_targets, num_total_anchors, inside_flags, fill=0)
            bbox_offset_targets   = unmap(bbox_offset_targets, num_total_anchors, inside_flags, fill=0)
            bbox_offset_weights   = unmap(bbox_offset_weights, num_total_anchors, inside_flags, fill=0)

        return (anchors, labels, label_weights, bbox_targets, bbox_weights,
                pos_inds, neg_inds, 
                refine_labels, refine_labels_weights, bbox_refine_targets, bbox_offset_targets, bbox_offset_weights, refine_pos_inds, refine_neg_inds) #####
