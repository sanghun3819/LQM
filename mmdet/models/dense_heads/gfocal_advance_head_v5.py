...
class GFocalAdvanceHeadv5(AnchorHead):
    ...
    def __init__(self,
        ...
        super(GFocalAdvanceHeadv5, self).__init__(num_classes, in_channels, **kwargs)

        self.sampling = False
        if self.train_cfg:
            self.assigner = build_assigner(self.train_cfg.assigner)
            # self.refine_assigner = build_assigner(dict(type='ATSSAssigner', topk=9, positive_iou_thr=-1))
            # self.refine_assigner = build_assigner(dict(type='MaxIoUAssigner', pos_iou_thr=0.6, neg_iou_thr=0.6, match_low_quality=False))
            
            # SSD sampling=False so use PseudoSampler
            sampler_cfg = dict(type='PseudoSampler')
            self.sampler = build_sampler(sampler_cfg, context=self)
            # self.refine_sampler = build_sampler(sampler_cfg, context=self)
    ...
    def _init_layers(self):
        ...
        # self.add_module("uncertainty_cls_branch", UncertaintyBranch(self.feat_channels, self.feat_channels, uncertain=True))        
        # self.add_module("uncertainty_cls_branch", SubBranch(self.feat_channels, self.feat_channels))        # sub-cls-branch 1x1, no attention
        self.add_module("uncertainty_reg_branch", UncertaintyBranch(self.feat_channels, 64, uncertain=True))
        ###########################################################################
    ...
    def forward_single(self, x, scale):
        ...
        ###########################################################################
        # cls refinement
        # # cls_refine = self.uncertainty_cls_branch(cls_feat, quality_score)
        # cls_refine = self.uncertainty_cls_branch(cls_feat)
        # for cls_layer in self.conv_cls_refine:
        #     cls_refine = cls_layer(cls_refine)
        # # cls_offset_score = self.conv_cls_refine_pred(cls_refine).sigmoid() * quality_score.detach()
        ...
        return cls_score, bbox_pred, bbox_offset                                                                                      
    ...
    def loss_single(self, anchors, 
        ...
        if len(pos_refine_inds) > 1:
            ...
            loss_cls_offset = self.loss_cls(cls_offset, (refine_labels, refine_score),
                                                    # weight=None,
                                                    # weight=refine_labels_weights,
                                                    # avg_factor=refine_valid_num + 1)
                                                    weight=refine_labels_weights,
                                                    avg_factor=num_total_refine_samples)
            # loss_cls_offset = self.loss_cls_offset(cls_offset, refine_labels,
            #                                         weight=refine_score,
            #                                         avg_factor=num_total_refine_samples)
        else:
            loss_cls_offset = cls_offset.sum() * 0
    ...
    def _get_target_single(self,
        ...
        # print('num_pos :', len(pos_inds), 'num_neg :', len(neg_inds))
        if len(pos_inds) > 0:
        ...
        # num_refine_valid_inds = torch.nonzero(refine_inside_flags, as_tuple=True)[0]
        if not refine_inside_flags.any():
            return (None, ) * 7
        
        # assign gt and sample anchors
        # bboxes = num_refine_valid_inds.new_zeros((num_refine_valid_inds.size(0), 4), dtype=torch.float)
        ...
        bbox_assign_result = self.refine_assigner.assign(bboxes, num_level_bboxes_inside,
                                             gt_bboxes, gt_bboxes_ignore,
                                             gt_labels)

        # bbox_assign_result = self.refine_assigner.assign(bboxes, gt_bboxes)
        bbox_sampling_result = self.refine_sampler.sample(bbox_assign_result, bboxes, gt_bboxes)
        ...
        if unmap_outputs:
            ...
            refine_labels         = unmap(refine_labels, num_total_anchors, refine_inside_flags, fill=self.num_classes)
            refine_labels_weights = unmap(refine_label_weights, num_total_anchors, refine_inside_flags, fill=0)
            refine_bbox_targets   = unmap(refine_bbox_targets, num_total_anchors, refine_inside_flags, fill=0)
            bbox_offset_targets   = unmap(bbox_offset_targets, num_total_anchors, refine_inside_flags, fill=0)
            bbox_offset_weights   = unmap(bbox_offset_weights, num_total_anchors, refine_inside_flags, fill=0)

        return (ret_anchors, ret_labels, ret_label_weights, ret_bbox_targets, ret_bbox_weights,
                pos_inds, neg_inds, 
                refine_labels, refine_labels_weights, refine_bbox_targets, bbox_offset_targets, bbox_offset_weights, 
